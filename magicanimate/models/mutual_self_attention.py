...
class ReferenceAttentionControl():
    ...
    def register_reference_hooks(
        ...
        def hacked_basic_transformer_inner_forward(
            ...
            if self.only_cross_attention:
                ...
                if MODE == "read":
                    ...
                    # emjay added -----------------------------------
                    self.bank2 = [rearrange(d.unsqueeze(1).repeat(1, video_length, 1, 1), "b t l c -> (b t) l c")[:hidden_states.shape[0]] for d in self.bank2]
                    # -----------------------------------------------

                    #### eq. (2)
                    # emjay modified --------------------------------
                    if hasattr(self, 'bank2') and (len(self.bank2)!=0):
                        hidden_states_uc = self.attn1(norm_hidden_states, 
                                                    encoder_hidden_states=torch.cat([norm_hidden_states] + [b1+b2 for b1, b2 in zip(self.bank, self.bank2)], dim=1),
                                                    encoder_hidden_states_value = torch.cat([norm_hidden_states] + self.bank, dim=1),
                    ...
                    # second stage: use ref. 
                    # train phase: use initialized uc_mask when we call .update()
                    if is_train and not is_second_stage:  
                        _uc_mask = self.cfg_uc_mask.clone()
                        assert hidden_states.shape[0] == _uc_mask.shape[0], f"in training, cfg_uc_mask is used for drop the reference images so that batch_size must be equal : {hidden_states.shape[0]} vs {_uc_mask.shape[0]}"                        
                    else:
                        _uc_mask = uc_mask.clone()
                    
                    # if _uc_mask is all false, tensor is empty => error
                    # do not include uncond during second_stage
        ...
        if self.reference_attn:
            ...
            for i, module in enumerate(attn_modules):
                ...
                if self.is_train:
                    # add cfg_uc_mask to BasicTransformerBlock
    ...
    def update_by_value(self, writer_output, ch_fixer=None, dtype=torch.float16):
        ref_down_block_res_samples, ref_mid_block_res_sample = writer_output
        # for r in ref_down_block_res_samples:
        # print(r.shape)
        # torch.Size([1, 320, 1, 16, 16])
        # torch.Size([1, 320, 1, 16, 16])
        # torch.Size([1, 320, 1, 16, 16])

        # torch.Size([1, 320, 1, 8, 8])
        # torch.Size([1, 640, 1, 8, 8])
        # torch.Size([1, 640, 1, 8, 8])

        # torch.Size([1, 640, 1, 4, 4])
        # torch.Size([1, 1280, 1, 4, 4])
        # torch.Size([1, 1280, 1, 4, 4])

        # torch.Size([1, 1280, 1, 2, 2])
        # torch.Size([1, 1280, 1, 2, 2])
        # torch.Size([1, 1280, 1, 2, 2])
        
        # for m in ref_mid_block_res_sample:
        #     print(r.shape)
        # torch.Size([1, 1280, 1, 2, 2])
        ...
        if self.reference_attn:
            ...
            #     ])
            # for r, w in zip(reader_attn_modules, writer_attn_modules):
            # print(r.bank[0].shape)
            # torch.Size([1, 4, 1280])
            # torch.Size([1, 16, 1280])
            # torch.Size([1, 16, 1280])
            # torch.Size([1, 16, 1280])
            # torch.Size([1, 64, 640])
            # torch.Size([1, 64, 640])
            ...
            # ch_fixer = ch_fixer.to(dtype)
            for r, w in zip(reader_attn_modules[1:], ref_down_block_res_samples[8::-1]):
                # r.bank = [v.clone().to(dtype) for v in w.bank]
                # w.bank.clear()
                b, s, c = r.bank[0].shape
                _, wc, wf, wh, ww = w.shape
                if w.shape[1] != c:
                    ch_fixer = ch_fixer.to(w.device)
                    # w = ch_fixer(w.permute(0,2,1,3,4).squeeze(dim=1))
                    w = ch_fixer(w.permute(0,2,1,3,4).reshape(-1, wc, wh, ww))
                    # w = w.unsqueeze(dim=2)                
                    w = w.reshape(b, -1, wf, wh, ww)

                r.bank2 = [w.permute(0,2,3,4,1).reshape(-1, s, c).clone().to(dtype)]
                # if self.is_train:
                #     r.cfg_uc_mask = cfg_uc_mask.clone()
