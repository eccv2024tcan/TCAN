...
from magicanimate.utils.util import get_tensor_interpolation_method, get_taumap
...
class AnimationPipeline(DiffusionPipeline):
    ...
    #### 2. controlnet : N timesteps, 12+1 feature maps
    @ torch.no_grad()    
    def run_controlnet(self, latents, t, num_context_batches, context_queue, context_batch_size, context_scheduler, context_frames, context_stride, context_overlap, 
                       controlnet_conditioning_scale, controlnet_res_samples_cache_dict, controlnet_text_embeddings_c, controlnet_cond_images, 
                       use_temporal_controlnet, num_inference_steps, device):        
        for i in range(num_context_batches):
            ...
            controlnet_latent_input = rearrange(controlnet_latent_input, "b c f h w -> (b f) c h w")  # [16 4 64 64]
            
            # controlnet inference
            down_block_res_samples, mid_block_res_sample = self.controlnet(
                
                controlnet_latent_input if not use_temporal_controlnet else controlnet_latent_input.permute(1,0,2,3)[None],
                t,
                encoder_hidden_states=torch.cat([controlnet_text_embeddings_c[c] for c in context]),  # [f 77 768]                
                controlnet_cond=torch.cat([controlnet_cond_images[c] for c in context]) if not use_temporal_controlnet else torch.cat([controlnet_cond_images[c] for c in context])[None],  # [f 3 512 512]                
                conditioning_scale=controlnet_conditioning_scale,
                return_dict=False,
            )  # controlnet 12 encoder feature map, 1 mid feature map
            
            if use_temporal_controlnet:
                mid_b, mid_c, mid_f, mid_h, mid_w = mid_block_res_sample.shape
                mid_block_res_sample = mid_block_res_sample.permute(0,2,1,3,4).reshape(-1, mid_c, mid_h, mid_w) # b c f h w -> bf c h w
                for i_db, db in enumerate(down_block_res_samples):
                    down_b, down_c, down_f, down_h, down_w = db.shape
                    down_block_res_samples[i_db] = db.permute(0,2,1,3,4).reshape(-1, down_c, down_h, down_w)
        ...
        return context_queue, controlnet_res_samples_cache_dict
    ...
    def __call__(
        ...
        use_temporal_taumap=False,  # if True, pose로 taumap 만들고 controlnet과 똑같이 샘플링
        use_temporal_taumap2=False, 
        use_temporal_controlnet = False,
        use_noisy_ref_img = False,
        **kwargs,
    ):
        """
        New args:
        ...
        #### cfg used for training is not activated cuz we are using this one during validation
        ...
        control = self.prepare_condition(
            ...
            ) # (48, 128, 128, 3) -> [96, 3, 128, 128]
        controlnet_uncond_images, controlnet_cond_images = control.chunk(2) # [n 3 512 512], 0~1
        ...
        if init_latents is not None:
            ...
            latents = self.prepare_latents(  # [b 4 f 64 64]
        ...
        _, controlnet_text_embeddings_c = controlnet_text_embeddings.chunk(2)  # [n 77 768]
        ...
        if isinstance(source_image, str):
            ...
            ref_image_latents = self.images2latents(source_image[None, :], latents_dtype).cuda()  # [1 4 64 64]
        
        context_scheduler = get_context_scheduler(context_schedule)
        ref_image_latents = ref_image_latents.repeat(context_batch_size * (2 if do_classifier_free_guidance else 1), 1, 1, 1)
        
        # Denoising loop
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps), disable=(rank!=0), leave=False):
            ...
            #### 1. appearance_encoder : save 10 feature maps in the self.bank
            
            if use_noisy_ref_img:
                ref_noise = torch.randn_like(ref_image_latents)
                ref_image_latents_input = self.scheduler.add_noise(ref_image_latents, ref_noise, t)
            else:
                ref_image_latents_input = ref_image_latents
            appearance_encoder(
            ...
            #### 2. controlnet : N timesteps, 12+1 feature maps            
            context_queue, controlnet_res_samples_cache_dict = self.run_controlnet(
                latents, t, 
                num_context_batches, context_queue, context_batch_size, context_scheduler, context_frames, context_stride, context_overlap, 
                controlnet_conditioning_scale, controlnet_res_samples_cache_dict, controlnet_text_embeddings_c, controlnet_cond_images, 
                use_temporal_controlnet, num_inference_steps, device)
            ...
            for context in global_context[rank::world_size]:
                ...
                #### 3. update : copy 10 feature maps from the appearance_encoder into the self.bank
                reference_control_reader.update(reference_control_writer)

                # predict the noise residual
                #### 4. add something that's copied from BasicTransformerBlock to self.bank
        
                if use_temporal_taumap or use_temporal_taumap2:
                    pose_lst = []
                    for i in np.concatenate(np.array(context)):
                        pose_lst.append(controlnet_condition[i])
                    temporal_taumap = get_taumap(pose_lst)
                    temporal_taumap = torch.from_numpy(temporal_taumap).type(latent_model_input.dtype).to(latent_model_input.device)

                pred = self.unet(
                    latent_model_input,  # [2 4 16 64 64]
                    t,  # [1]
                    encoder_hidden_states=text_embeddings[:b],  # [2 77 768]
                    down_block_additional_residuals=down_block_res_samples,  
                    mid_block_additional_residual=mid_block_res_sample,
                    return_dict=False,
                    temporal_taumap=temporal_taumap if use_temporal_taumap else None,
                    temporal_taumap2=temporal_taumap if use_temporal_taumap2 else None,
                )[0]
            ...
            #### 6. flush appearance_encoder's bank 
