...
def model_load(p):
    ...
    if ext == ".ckpt" or ext == ".pth" or ext == ".bin":
        state_dict = torch.load(p, map_location="cpu")
    elif ext == ".safetensors":
        state_dict = load_file(p)
    else:
        assert False, f"invalid path {p}"
    new_state_dict = OrderedDict()
    for k, v in state_dict.items():
        new_state_dict[k.replace("module.", "")] = v.clone()
    del state_dict
    return new_state_dict
def save_args(args, to_path):
    with open(to_path, "w") as f:
        json.dump(args.__dict__, f, indent=2)
def save_config(config, to_path):
    OmegaConf.save(config, to_path)
def none_filter(batch):
